...
def test_promo_code_generation_default_values(models_data, mocker, app):
    ...
    mocker.patch.object(domain.utils,
                        'string_generator',
...
def test_promo_code_generation(models_data, mocker, app):
    ...
    with app.app_context():
        ...
        assert response_code == GeneratePromoCodeResult.EventDoNotExists
        event_id = 1
        response_code, res = prom_code.generate_promo_code(
            event_id, 2, {
                "credit": CREDIT,
                "radius": radius,
                "event_id": event_id,
...
def test_promo_code_generation_default_expiration_time_length(
...
def test_promo_code_lists(models_data, mocker, app):
    ...
    with app.app_context():
        ...
        assert set([a.code
                    for a in actives]) == set([pcode2[0].code, pcode3[0].code])

        all_codes = prom_code.get_all_promo_codes(1)
        assert set([a.code for a in all_codes
                    ]) == set([pcode2[0].code, pcode3[0].code, pcode.code])
...
def test_valid_promo_code_acording_distances_when_radius_change(
        models_data, app):
    """
        Only valid when user’s pickup or destination is within x radius of the
        event venue
...
def test_flask_prom_code_generate_ok(models_data, mocker, client, app):
    ...
    mock_code = "mock_code"
    mock_response = GeneratePromoCodeResult.Ok, [
        PromoCodeResultTuple(mock_code)
    ]
    mocker.patch.object(domain.prom_code,
                        'generate_promo_code',
                        return_value=mock_response)
...
def get_ride_from_prom_code_error(models_data, mocker, client, app,
                                  mock_response_code):
    """
        To test the validity of the promo code, expose an endpoint that accept
        origin, destination, the promo code. The api should return the promo
...
def test_flask_get_ride_from_prom_code_PromCodeDoNotExists_error(
        models_data, mocker, client, app):
    get_ride_from_prom_code_error(models_data, mocker, client, app,
...
def test_flask_get_ride_from_prom_code_ok(models_data, mocker, client, app):
    RideFromPromCodeResultTuple = namedtuple(
        'RideFromPromCodeResultTuple', 'code credit event_id '
        'expiration_time ')
    """
        To test the validity of the promo code, expose an endpoint that accept 
        origin, destination, the promo code. The api should return the promo 
    ...
    mock_response = RideFromPromCodeResult.Ok, {
        "code":
        RideFromPromCodeResultTuple(mock_code, mock_credit, mock_event_id,
